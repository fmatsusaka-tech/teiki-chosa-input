...
| 処理区候補は`調査データ`の園地名が一致する行だけから取得し、候補外も自由記入できる | `survey-treatment-suggestions.test.ts`、`treatment-input.test.ts` |
| 処理区候補の取得で過去行を更新せず、取得不能時も自由記入と保存を妨げない | API実装確認、入力画面回帰確認 |

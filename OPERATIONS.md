...
メモ入力後に自動解析が始まらない場合は「解析を開始」を押します。「解析しています…」の後に
解析件数または入力確認の案内が表示されます。このボタンではSpreadsheetへ保存されません。
...
4. 連続して間違ったパスワードを入力すると、ブルートフォース対策により一時的にログインがロックアウトされる（HTTP 429）。数分待ってから再試行する。この状態はRenderプロセスのメモリ上の一時的なものであり、再デプロイまたは時間経過で解除される。
